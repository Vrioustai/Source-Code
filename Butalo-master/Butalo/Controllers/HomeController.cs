...
using Butalo.Models;
using Butalo.Services;

namespace Butalo.Controllers
{
    public class HomeController(IButaloRepository repository) : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            ViewData["LoaiCanHo"] = await repository.LayLoaiCanHoAsync();
            return View("trang_chu", dsPhong);
        }

        public async Task<IActionResult> Listings([FromQuery] TimKiemCanHoViewModel boLoc)
        {
        ...
        public async Task<IActionResult> Detail(string id)
        {
            if (string.IsNullOrWhiteSpace(id))
            {
                ...
                id = dsPhong.FirstOrDefault()?.MaCanHo ?? string.Empty;
            }
        ...
        public async Task<IActionResult> CreatePostForm()
        {
            ...
            var model = new TaoTinCanHoViewModel
            {
        ...
        public async Task<IActionResult> CreatePost(TaoTinCanHoViewModel model, IFormFile? anhPhong, [FromServices] IWebHostEnvironment env)
        {
            if (string.IsNullOrWhiteSpace(CurrentUserId))
            {
                TempData["ErrorMessage"] = "Vui lòng đăng nhập để đăng tin.";
            ...
            var MaCanHoMoi = await repository.TaoTinPhongAsync(model);
            if (anhPhong != null && anhPhong.Length > 0)
            {
                var duongDan = await repository.UploadAnhAsync(anhPhong, env);
                await repository.LuuAnhVaoDbAsync(MaCanHoMoi, model.MaTaiKhoan, duongDan);
            }

            TempData["SuccessMessage"] = $"Đăng tin thành công. Mã phòng: {MaCanHoMoi} (chờ duyệt)";
            return RedirectToAction(nameof(Detail), new { id = MaCanHoMoi });
        }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> EditPost(TaoTinCanHoViewModel model)
        {
            if (string.IsNullOrWhiteSpace(CurrentUserId))
            {
                TempData["ErrorMessage"] = "Vui lòng đăng nhập để chỉnh sửa tin.";
            ...
            {
                await LoadDanhMucAsync(model.MaTinhThanhPho, model.MaQuanHuyen);
                ViewData["PostId"] = model.MaCanHo ?? string.Empty;
                return View("chinh_sua_tin_dang", model);
            }

            var current = await repository.LayTinPhongDeSuaAsync(model.MaCanHo ?? string.Empty);
            if (current is null)
            {
                TempData["ErrorMessage"] = "Không tìm thấy tin cần chỉnh sửa.";
                return RedirectToAction(nameof(Listings));
            ...
            {
                TempData["ErrorMessage"] = "Bạn không có quyền chỉnh sửa tin này.";
                return RedirectToAction(nameof(Detail), new { id = model.MaCanHo });
            }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> DeletePostFromReport(string MaCanHo)
        {
            if (!string.Equals(CurrentRole, "QuanTri", StringComparison.OrdinalIgnoreCase))
            {
                TempData["ErrorMessage"] = "Bạn không có quyền truy cập chức năng này.";
            ...
            await repository.XoaPhongAsync(MaCanHo);
            TempData["SuccessMessage"] = $"Đã xóa bài đăng {MaCanHo} và các tố cáo liên quan.";
            return RedirectToAction(nameof(ReportManagement));
        }

        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> ApprovePost(string MaCanHo)
        {
            if (!string.Equals(CurrentRole, "QuanTri", StringComparison.OrdinalIgnoreCase))
            {
                TempData["ErrorMessage"] = "Bạn không có quyền truy cập chức năng này.";
            ...
            await repository.CapNhatTrangThaiDuyetTinAsync(MaCanHo, "KD001", true);
            TempData["SuccessMessage"] = $"Đã duyệt tin {MaCanHo}.";
            return RedirectToAction(nameof(Moderation));
        }

        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> RejectPost(string MaCanHo)
        {
            if (!string.Equals(CurrentRole, "QuanTri", StringComparison.OrdinalIgnoreCase))
            {
                TempData["ErrorMessage"] = "Bạn không có quyền truy cập chức năng này.";
            ...
            await repository.CapNhatTrangThaiDuyetTinAsync(MaCanHo, "KD003", false);
            TempData["SuccessMessage"] = $"Đã từ chối tin {MaCanHo}.";
            return RedirectToAction(nameof(Moderation));
        }
